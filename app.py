...
/* กล่อง Metric - ปรับแต่งสีตัวเลขให้เข้มขึ้นเพื่อให้ตัดกับพื้นหลังขาว */
...
/* --- 🌈 กำหนดสีตัวเลขพาสเทลแบบเข้ม (ตัดกับพื้นหลังขาว) --- */
div[data-testid="column"]:nth-child(1) div[data-testid="stMetricValue"] > div { color: #7c4dff !important; } /* ม่วง */
div[data-testid="column"]:nth-child(2) div[data-testid="stMetricValue"] > div { color: #f06292 !important; } /* ชมพู */
div[data-testid="column"]:nth-child(3) div[data-testid="stMetricValue"] > div { color: #00acc1 !important; } /* ฟ้าเขียว */
div[data-testid="column"]:nth-child(4) div[data-testid="stMetricValue"] > div { color: #43a047 !important; } /* เขียว */
div[data-testid="column"]:nth-child(5) div[data-testid="stMetricValue"] > div { color: #fb8c00 !important; } /* ส้ม */
...
with col_t:
    ...
    st.markdown(f"🌸 **สถานะ:** ทำงานปกติ  |  🕒 **อัปเดตล่าสุด:** {now_th.strftime('%H:%M:%S')} น.")
with col_b:
    st.write("##")
...
try:
    ...
    if not df.empty:
        ...
        # --- 4. Metric Grid (5 กล่อง - ภาษาเต็มรูปแบบ) ---
        m1, m2, m3, m4, m5 = st.columns(5)
        m1.metric("💰 ยอดรวมทั้งหมด (บาท)", f"฿{total_thb:,.2f}")
        m2.metric("🧠 ค่าใช้จ่าย Gemini (USD)", f"${gcp_usd:,.2f}")
        ...
        # --- 5. Main Content Area (Layout 3 คอลัมน์) ---
        # ปรับสัดส่วนให้กล่องสรุปรายเดือนเด่นขึ้น
        ...
        with l_col:
            with st.container(border=True):
                ...
                st.area_chart(chart_df['Total Cost (THB)'], color="#7c4dff")
